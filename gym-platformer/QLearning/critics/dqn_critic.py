...
import sys
from QLearning.infrastructure.dqn_utils import minimize_and_clip, huber_loss
...
class DQNCritic(BaseCritic):
    ...
    def _build(self, q_func):
        ...
        if self.double_q:
            ...
            q_argmax = tf.argmax(self.q_t_values, axis=1, output_type=tf.int32)
            arr = tf.range(tf.shape(q_argmax)[0], dtype=tf.int32) 
            indices = tf.stack([arr, q_argmax], axis=1) 
