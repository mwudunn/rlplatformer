...
from QLearning.infrastructure.tf_utils import build_mlp
    ...
    def update(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for i in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            if i % self.num_grad_steps_per_target_update == 0:
                v_s_prime = self.forward(next_ob_no) * (1 - terminal_n)
                t_values = re_n + self.gamma * v_s_prime
            _, loss = self.sess.run([self.critic_update_op, self.critic_loss], feed_dict={self.sy_ob_no: ob_no, self.sy_target_n: t_values})
        return loss
